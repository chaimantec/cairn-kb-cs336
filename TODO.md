...
## Run 19 — Lecture 18: Guest Lecture (Dan Fu) — Serving, Megakernels, Looped Recurrence

Video 9EEm4iMAF5s (72 min). **THE LAST LECTURE.** Catalog position 18; the
course site's own schedule calls it **session 19** (Wed June 3) — session 18 was
a second guest lecture (Daniel Selsam) that was never recorded. Record the
discrepancy, do not absorb it silently.

### Course material — NONE EXISTS (verified, not assumed)
- [x] Applied the "list the directory" rule rather than a filename search:
      - the schedule's two guest-lecture rows have EMPTY materials cells;
      - `stanford-cs336/lectures` tree = 425 paths, no lecture_18/19, nothing
        matching fu/selsam/guest;
      - sibling archives `spring2025-lectures` and `spring2024-lectures` both
        publish decks under `nonexecutable/` — neither has a speaker-named file
        (this is exactly where CS224N run 12's two missing decks were hiding);
      - `stanford-cs336.github.io` carries no docs at all.
      CONCLUSION: transcript-only, and the only lecture in this KB with no
      images. NOTE from the lecture itself: the speaker says his slides were
      generated by Nano Banana Pro and that one of them HALLUCINATED its
      content — so even the unpublished deck is not a citable authority.

### Transcript
- [x] 18 — verbatim captions fetched (1914 segments, 93 markers, ~13.8k words),
      saved to the gitignored raw/transcripts/original/ with its segment JSON
- [x] 18 — copy-edited transcript (Sonnet, 93 paragraphs, 11.9k words from
      13.8k of captions). Appended in batches throughout; not killed.
- [x] 18 — verify: ALL THREE AVAILABLE CHECKS PASS. The fourth (proper-noun
      grep against the deck) is IMPOSSIBLE for this lecture — there is no deck.
      Timestamps: 93 markers, sequence identical, none moved.
      Numbers: after fixing two checker bugs, 10 lost / 1 gained. Every one
      adjudicated: "NVL sendme 2" -> NVL72 (the next sentence says "these are
      72 GPUs"), and the rest are DIGITS SPELLED AS WORDS with content intact —
      "Ten years later, by 1912, cars had already outnumbered horses",
      "that ten-year transition", "nine times out of ten", "five to ten
      minutes", "one plus one equals two" (the original itself spells "equals
      two" later in that same sentence). Same class as CS224N's precedent.
      Ratios: 1 of 93 outside the band ([16:13], 0.71) and it is pure filler
      removal — an abandoned false start ("I'll get into a little bit of"), a
      self-correction ("this phenomenon called - this technique called") and
      doubled articles. Nothing substantive lost.
      NO HEADER CORRECTION NEEDED, unlike runs 17 and 18: the header's "nothing
      added, removed or reordered" survives scrutiny because every [Ed: unclear]
      marker PRESERVES the as-heard text inside it rather than replacing it.
      Adjudicated the riskiest restorations by hand, since no deck can confirm
      them: "micro kernels" -> megakernels is sound (the speaker's very next
      clause says "mega kernels"); "life of a kernel" -> "life of a token" is
      sound ("lifetime of a token" appears verbatim twice earlier); "Grock" ->
      Groq not Grok, confirmed by "the LPU" in the same sentence. Eight spots
      left [Ed: unclear] rather than guessed, including "at Ender" (plausibly
      "at Together", his employer — NOT substituted, which is exactly the
