...
audit: >
  THREE PASSES DONE. 36 of the 57 pages have been fully audited against the
  rendered PDF by delegated readers, plus existence-only checks on 17, 19,
  27 and 28 (all four correct — those rasters are pasted equations and paper
  headers, not undescribed figures), and a partial check on 44.
  ...
  six. Pass 3 ran in seven batches over 7, 8, 9, 10, 16, 22, 23, 25, 30, 32, 33,
  35, 36, 37, 39, 43, 47, 53 and found 48 more on 15 of those 18.
  Combined: TWENTY-SIX of thirty-six audited pages carried errors, 84 in all,
  every one now corrected below. Page 44 is a separate, partial case: panel (a)'s
  seven-row fit/extrapolate grid was verified cell by cell while checking the
  committed slide images and is exact, but panels (b) and (c) were only read at
  page scale, so treat their values as unaudited.
  PASS 3's own headline: pages 43, 47 and 53 were all three dirty, 12 errors.
  Two more undisclosed logarithmic y-axes turned up (43's Loss, 47's Training
  loss), taking this deck to SEVEN axes whose scale the transcription did not
  state or got wrong — by a wide margin the most repeated error class here, and
  the one to check first on any page of this deck. Page 53 also produced this
  deck's THIRD internal contradiction: its Hoffmann residual range stopped at
  +0.06 while its own axis note said points sit above 0.10. Checking an entry
  against itself remains free and remains productive. This is by a wide margin the dirtiest deck in
  ...
  CROSS-SLIDE NOTE, unresolved. Slides 36 and 55 both carry the Abnar et al. 3D
  "IsoFLOP surface over sparsity and active parameters" figure, but they embed
  DIFFERENT image objects (the extracted streams have different MD5s), so they are
  different crops or renderings rather than one reused asset. Their sparsity tick
  lists accordingly differ and have NOT been reconciled: slide 36's list was measured
  at audit and reads 0/39/63/78/86/92/95/97/98%, while slide 55's entry gives
  0/39/63/78/86/91/95/98%. Both may be correct for their own crop; slide 36's is the
  one that has been measured. Do not assume either is wrong without re-reading both.
...
- **1 Layer**: starts earliest/leftmost, at $x\approx7\times10^2$, $y\approx6.55$; descends steadily as the topmost (worst-loss) curve; passes about ($3\times10^6$, 4.8); ends at $x\approx4.8\times10^7$, $y\approx4.28$ — it stops well before the other series and does not reach the right side of the plot.
...
- **3 Layers**: starts at $x\approx8.9\times10^3$, $y\approx6.25$ (essentially tied with 2 Layers' start height, at a slightly larger x); passes about ($4\times10^4$, 5.74), ($1.7\times10^5$, 5.25), ($7\times10^5$, 4.76); ends at $x\approx1.37\times10^8$, $y\approx3.38$ — it extends slightly further right, and ends slightly lower, than 2 Layers.
...
- **test error** (circles, with visible error bars): a cluster of points connected by a solid line ("points used for prediction") for the first several points, falling steeply from about (2560, 0.27) down to about (2560+, 0.09); the line then continues as a dashed "predicted learning curve" for the remaining points, staying roughly flat around 0.08 out to the rightmost plotted point beyond 15360. **Marker fill changes along the series**: only the first five or six points — the steep initial drop from ≈0.27 to ≈0.09 — are open/hollow circles. From the point where the "test error" label sits onward, through the rest of the solid segment and the whole dashed segment, the circles are solid black.
- **training error** (triangles): starts near (2560, 0.01) and rises with a solid "points used for prediction" line to about 0.06; continues as a dashed "predicted learning curve," staying roughly flat and rising very slightly to about 0.07 by the rightmost point. **Every training-error marker is an OPEN (hollow, white-interior) triangle**, including the dense cluster near the origin — that cluster only reads as solid because many overlapping outlines are packed together; any well-separated triangle, around $x\approx5000$–$8000$, is plainly hollow.
...
- **Winnow** (blue, "X" markers): starts lowest of all series, at $x\approx0.2$, $y\approx0.75$; rises steeply to catch up by $x=1$ ($y\approx0.83$); at $x=10$, $y\approx0.91$ — where it sits inside a tight three-way bundle with Perceptron (≈0.91) and Naïve Bayes (≈0.92), not above them; it reaches ≈0.95 only around $x\approx50$ and holds that plateau through $x=100$–200; ends at $x=1000$, $y\approx0.97$ — the highest final value of the four series. (Values here were fixed by pixel-calibrated measurement against the log axis at audit; the earlier reading placed the 0.95 plateau a full data point too early, at $x=10$, and recorded a dip to 0.94 at $x=100$ that does not exist.)
...
**Figure 1 continued (middle) — token error rate learning curve.** Same y-axis style, "Minimum Test Loss (Log-scale)", ticks 0.36 through 0.71; x-axis "Training Data Set Size, Number of Tokens (Log-scale)", ticks $2^{19}$ through $2^{27}$, all cleanly visible and evenly spaced — the panel is NOT cropped, contrary to an earlier reading of it. Two series per the legend: **Token Error Rate** (blue solid) and **Token Error Rate Trend** (orange dashed), nearly coincident. The solid curve descends from about ($2^{19}$, 0.71) to about ($2^{27}$, 0.39), and the dashed trend line runs slightly below it at the largest data sizes, ending around ($2^{27}$, 0.36). Annotated fit: $\varepsilon(m) = 3.87\,m^{-0.13}$.
...
*Panel 1 — Compute.* Y-axis "Test Loss", **log** scale, ticks 2 through 7. (Verified by measurement, not by eye: the inter-tick pixel gaps run 104, 119, 147, 188.5 — monotonically increasing — and normalising each by the log of its tick ratio gives a constant ≈1500-1555 px per decade. The shared 5.6/4.8/4.0/3.2/2.4 tick column used by panels 2 and 3 gives ≈1677 px per decade with under 0.5% variance. All three Figure-1 panels share this log Test-Loss axis.) X-axis "Compute" (subtitle "PF-days, non-embedding"), log scale, ticks $10^{-9}$ through $10^{1}$. Body: a dense fan of light-blue individual training-run curves (same style as slide 4's left panel), a black lower-envelope line tracing the minimum loss achieved at each compute budget, and a dashed gold/orange fit line labelled "$L = (C_{min}/2.3\cdot10^{8})^{-0.050}$" that tracks the black envelope closely across the full range, from about ($10^{-9}$, 7.0) — its left end meets the top border of the plot, on the $y=7$ gridline — down to about ($10^{1}$, 2.4). (It is the black envelope curve, not this dashed fit, that begins near 6.4-6.5.)
...
- **q=0.22** (orange): from about $(10^2, 0.06)$ down to about $(5\times10^3, 0.001)$, below the blue line throughout.
- **q=0.56** (green): from about $(10^2, 0.04)$ down to about $(5\times10^3, 0.00065)$, the lowest line throughout, tracking just under the orange line for most of its length.

  (Both endpoints were corrected at audit; an earlier reading overstated them by roughly 4x, at 0.004 and 0.003. The measured values are also the internally consistent ones: they put all three series' log-log slopes at about $-0.94$, $-1.00$ and $-0.98$ — closely parallel, which is exactly what this slide's own text asserts. The overstated figures would have implied slopes near $-0.69$ and $-0.66$ for orange and green, contradicting the slide.)
...
- **Models trained** (points): seven points — one at 12B plus six clustered between roughly 30B and 90B (the six magnified in the inset) — running from $(12B, \approx2.85)$ down through the 30–90B range (detailed in the inset, roughly 2.69 down to 2.52) to a rightmost plotted point a little past 48B.
...
**Chart 2 (right) — "Allocating compute when repeating."** Y-axis: "Parameters", with two labelled values, 8.67B (upper) and 6.34B (lower). X-axis: "Tokens (Epochs)", with two labelled value-pairs, 178B (7.1) and 242B (9.7). **Both axes are log scale.** The chart prints no "log" label and carries only two ticks per axis, so this was established by calibration rather than read off: extrapolating the two known ticks linearly to the plot's origin corner yields impossible negative values (≈ −3.8B parameters, ≈ −172B tokens), while a log assumption yields plausible minima near 1.6B parameters and 33B tokens. It is corroborated by the isoFLOP line itself — $6 \times 8.67\text{B} \times 178\text{B}$ and $6 \times 6.34\text{B} \times 242\text{B}$ both come to ≈$9.2\times10^{21}$, matching the line's own "$10^{22}$ FLOPs" label, and that line is straight only in log-log space. Legend (beneath the chart): blue dashed, "Regime of same compute (IsoFLOP)"; solid black, "Efficient frontier assuming repeated data is worth the same as new data"; a black-to-red-to-orange gradient solid line, "Efficient frontier predicted by our data-constrained scaling laws". The blue dashed line, annotated "$10^{22}$ FLOPs", runs from upper-left to lower-right across the whole plot. The black solid line and the gradient line both rise from the origin (bottom-left corner) toward the upper-right, the black line at a steeper angle than the gradient line. A gold/yellow star, labelled "Loss: 2.376", sits where the black line crosses the blue dashed line, at the 178B(7.1) tokens position and a parameters value near (but at this resolution not exactly on) the 8.67B gridline. A red star, labelled "Loss: 2.359", sits where the gradient line crosses the blue dashed line, at the 242B(9.7) tokens position and a parameters value near the 6.34B gridline. The point of the chart: at the same fixed compute budget ($10^{22}$ FLOPs), the data-constrained scaling law's efficient frontier (gradient line, ending at loss 2.359) reaches a lower loss than the naive frontier that treats repeated data as equally valuable as new data (black line, loss 2.376), by choosing a smaller model trained on more token-epochs.
...
**Figure 3 — "Best Data Pool Changes with Total Compute."** Three stacked, dashed-outline boxes. Top, pale-green outline, labelled "Small Compute": two small boxes, "Epoch 1: E" (dark green, RGB≈(2,90,50)) and "Epoch 2: E" (a distinctly lighter medium green, RGB≈(66,171,93)) — the same per-epoch lightening used elsewhere in the figure, rather than two boxes of one shade. Middle, pale-blue outline, labelled "Medium": two green boxes both reading "E + D". Bottom, pale-red outline, labelled "Large": two green boxes both reading "E + D + C".
...
- **Actual** markers (matching each series' colour and shape) sit on or near their own line at a few sampled x-positions (roughly $x\approx40$, $x\approx130$, and $x\approx600$); one exception is an isolated green "Actual" circle near $x\approx600$ that sits noticeably **below** the flat green "Estimated" line at that point (marker ≈0.68 against the line's ≈0.69 — a lower error value, and so lower on the chart).
...
- Blue "Transformers": the single lowest curve on the plot. Runs the full width of the chart. It is clipped by the left plot boundary at about $3\times10^4$, with its first actual marker around $(5.5\times10^4, 4.9)$, and descends to about $(1.4\times10^9, 2.33)$.
- Light-pink "1 Layer" (LSTM): starts at about (1×10^5, 4.95), overlapping closely with "2 Layers" at the start, and ends around $(7.6\times10^7, 3.35)$ — the worst (highest-loss) of the three LSTM curves at the point where it stops, and **the leftmost-stopping of the three**.
- Medium-red "2 Layers" (LSTM): starts at about (1×10^5, 4.95) alongside "1 Layer", tracks just below it, and continues further right than the other two LSTM curves, ending around $(2.1\times10^8, 3.16)$ — the rightmost endpoint of the LSTM group, at essentially the same loss as "4 Layers" rather than below it.
- Dark-maroon "4 Layers" (LSTM): joins the plot later than the other two, around $(2.4\times10^6, 3.96)$, and ends around $(1.2\times10^8, 3.16)$ — stopping *between* the other two, not before them.

  (Three of these four readings were corrected at audit. The stopping order left to right is 1 Layer, then 4 Layers, then 2 Layers; an earlier reading called 4 Layers the leftmost-stopping curve, which its own recorded coordinates already contradicted. The two deepest LSTMs finish tied at ≈3.16 rather than 2 Layers finishing lower, and the Transformers curve begins three to five times further left than first recorded.)
...
**Panel (b), "Optimal Total Parameters $N^*$":** x-axis "Total Parameters $N$", log scale, ticked 178M, 485M, 1B, 4B, 10B, 26B; y-axis "Pretraining Loss $L$" — the same label panel (c) carries — ticked 2.2, 2.4, 2.6, 2.8, 3.0. (An earlier reading called these numerals illegible; they read cleanly at 1200 dpi. In this deck every claim of illegibility that has been tested has proved false.). A family of U-shaped curves, one per MoE sparsity level, coloured from orange (0% sparsity) through pink/magenta to dark purple (95% sparsity), per a colour-bar legend below the panel ("MoE Sparsity $S$", 0% to 95%). A separate green curve traces the lower envelope across all the sparsity curves, bottoming out around (10B, its minimum). White star markers ("Optimal total parameters", per the panel's own legend) trace one point on each sparsity curve, forming a diagonal band running from the upper-left down to the green envelope's minimum as sparsity increases.

**Panel (c), "Optimal Active Parameters $N_a^*$":** x-axis "Active Parameters $N_a$", log scale, ticked 178M, 294M, 485M, 800M, 1B, 2B, 4B (seven ticks — an earlier reading omitted the rightmost); y-axis "Pretraining Loss $L$", linear, ticked 2.2 to 2.7. The same family of sparsity-coloured U-shaped curves (orange 0% through purple 95%) plus the same style of green envelope curve, whose minimum sits around (560M, 2.2) — left of the 800M tick, roughly a third of the way from 485M toward it. White star markers ("Optimal active parameters") again trace a band across the family, running from about (1.3B, 2.44) at low sparsity — visibly right of the 1B tick — down to the green curve's minimum at high sparsity.

**A third, smaller panel — "(b) IsoFLOP surface over sparsity and active parameters" — a 3D surface plot**, positioned below/right of the two 2D panels. Axes: "Active Parameters $N_{active}$" (log scale, floor axis, ticked 10B, 6B, 4B, 2B, 1B, 800M, 485M, 294M, 178M, 108M, 66M, decreasing left to right), "MoE Sparsity $S$" (floor axis, ticked 0%, 39%, 63%, 78%, 86%, 92%, 95%, 97%, 98%, increasing toward the back-right; four of these were each off by one point in an earlier reading, and all nine read clearly under high zoom with unsharp masking), "Loss $\mathcal{L}$" (vertical, ticked 2.2, 2.3, 2.4, 2.5, 2.6, 2.7, 2.8 — seven ticks, with NO 2.1 tick on the axis itself; 2.1 appears only as the low end of the separate colour-bar legend at the top of the slide, also labelled "Loss $\mathcal{L}$" and running 2.1 to 2.8). A grey saddle/bowl-shaped surface is drawn across the floor grid, and overlaid on it are strings of coloured dots (coloured by sparsity, same orange-to-purple scale as the other two panels, per a "Sparsity $S$" colour-bar legend at top, 0% to 98%) connected by dashed lines — one string per sparsity level — each string forming its own shallow U-shaped valley in active parameters, with the valley's minimum shifting toward fewer active parameters and lower loss as sparsity increases from 0% (orange, valley around 2.5–2.6) to about 93–96% (dark purple, valley around 2.15–2.2).
...
- Light-green dots, "Noise Scale Measurement": several hundred individual scattered points, densest in a tight vertical cluster at the far left (around x=10^1, spanning y≈800 to 4000) and then spreading out with increasing scatter as x decreases (moving right), tracking the general upward trend up to a highest point of about $9.4\times10^5$ (at loss ≈2.5) — no green point reaches $2\times10^6$ anywhere on the chart.
- Blue line+markers, "Empirical $B_{crit}$, $N=3M$": a smoothed trace through the scatter, starting around $(10^1, 2.7\times10^3)$ and rising essentially monotonically from its first point — it does NOT dip early; that is the orange series. It spikes sharply to about $7\times10^5$ partway along, then drops to one further point and TERMINATES at about (loss ≈3.5, $4.8\times10^5$), short of the $3\times10^0$ tick. (Three corrections at audit: the early dip belongs to orange, the spike peak was understated at $4\times10^5$, and the stated endpoint of $(3\times10^0, 4\times10^5)$ was actually the ORANGE line's final point, duplicated here.)
- Orange line+markers, "Empirical $B_{crit}$, $N=85M$": a similar smoothed trace, starting around $(10^1, 3.6\times10^3)$, dipping to a low of about $2.8\times10^3$ around loss ≈8.5–9 before climbing steadily and peaking near (3×10^0-ish, just under 10^6), then dropping back at its very last point to about $4.15\times10^5$.
- Grey dashed line, "$B_{crit} = 2.1\times10^8$ tokens $\cdot L^{-4.8}$": a straight power-law reference line spanning the full plot width, from about $(10^1, 3\times10^3)$ to about (loss ≈2.5, $2.4\times10^6$) — its upper-right terminus sits right of the $3\times10^0$ tick and short of both the top and right plot borders.
...
**Figure 1 (top right) — "Loss vs Model and Dataset Size" log-linear line chart.** Y-axis "Loss", **logarithmic** — ticked 2.5, 3.0, 3.5, 4.0, 4.5, and the gaps between consecutive ticks grow steadily down the axis (515, 594, 701, 808 px at 2400 dpi). Fitting the tick pixel positions gives R^2 = 0.99997 for a log axis against 0.9920 for a linear one, so this is log, not the linear scale the unqualified tick list would imply, X-axis "Tokens in Dataset", log scale, from about 10^7 to beyond 10^10. Six dotted series with circular markers, per the legend "Params": yellow "708M", light-green "302M", teal-green "85M", dark-blue "3M", teal "25M", dark-purple "393.2K" (the legend's own ordering, not sorted by parameter count). Every curve starts near the same loss at the smallest dataset size (upper left, around 4.1–4.7 — measured through the log calibration: 85M 4.12, 25M 4.17, 708M 4.27, 3M 4.48, 393.2K 4.69) and descends as dataset size grows, then flattens into its own horizontal plateau toward the right; the plateau height is set by model size — the two largest-parameter curves (yellow 708M, light-green 302M) plateau lowest (around 2.5), while the smallest model (dark-purple 393.2K) plateaus highest (measured 4.22 at the rightmost token count; the same value under either a log or a linear reading of the axis) and barely descends at all across the same token range.

**Figure 2 (bottom right) — "(a) Wiki103 error (cross entropy) landscape" 3D surface plot [Rosenfeld+ 2020].** Axes: "log2(data fraction)" (one floor axis, ticked -5, -4, -3, -2, -1, 0), "log2(model fraction)" (the other floor axis, ticked 0, -2, -4, -6, -8, -10, -12), "log10(err)" (vertical axis, ticked 0.55 to 0.80). A saddle-shaped 3D surface is drawn, coloured on a blue (low error) to red/orange (high error) gradient. The high-error red ridge traces down the floor gridlines to **log2(data fraction) = -5**, the smallest data slice, so the low-error blue corner is the **full-data** end of that axis, not the less-data end; error falls as both data and model grow, which is the slide's own thesis. (The model-fraction direction is genuinely hard to read off this projection; the data-axis reading is a direct gridline trace, the model half follows from monotonicity), with small purple dot markers tracing sampled points across the surface's ridge. Additional 2D projections of the same data are drawn flattened against the walls of the plot's bounding box: a family of nearly-horizontal red-toned curves projected onto the upper back wall, and a family of blue- and red-toned curves projected onto the floor of the box.
...
**Left panel — "IsoLoss contours", titled above the plot.** X-axis "Training FLOPs", log scale, ticks $10^{18}$ through $10^{23}$, plus a labelled dashed vertical grey line at the right edge marked "Gopher budget". Y-axis "Model size", log scale, ticks 100M, 1B, 10B, 100B. A horizontal dashed grey line runs across the panel at $y=40\text{B}$; together with the vertical "Gopher budget" line it forms a crosshair marking the caption's projected optimum of 40B parameters at Gopher's compute budget. The plot area is filled with many curved iso-loss contour lines, colored by a pale-to-black "Loss" colormap (the colorbar sits BETWEEN the two panels — to the right of this contour panel, not at the far right of the figure, where the right panel's own "Train. FLOPs" series legend sits. It runs from a pale top tick of ≈5.00, through red/orange around 3.5-4.0 and magenta/purple, down to near-black ≈2.00; lower loss = darker). Overlaid are:
...
- A solid blue line, "Efficient frontier" — a straight line in this log-log space, running from about ($2\times10^{18}$, 100M) up to its terminus at the Gopher-budget/40B crosshair — roughly $5\times10^{23}$–$10^{24}$ FLOPs at 40B parameters — passing through each iso-loss contour at its minimum-FLOPs point. That endpoint is the panel's whole point: it is the "40B parameters" the caption projects.
...
- One visually distinct hollow/open-outline (white-centred) data point near $10^{19}$ FLOPs, at roughly 7-8B parameters, standing out from the solid-filled empirical-data dots around it; the slide gives no caption explaining it, so it is noted here as visually distinct without a stated meaning. (A nearby point around 3B is *not* hollow — it carries a pale cream/peach fill, which is simply its high-loss colour on the colormap, in the same filled-marker style as the rest of the empirical data.)

**Right panel — "IsoFLOPs slices", titled above the plot, with its own legend.** X-axis "Model size", log scale, ticks 100M, 1B, 10B, 40B. Y-axis "Loss", ticks 2.00, 3.00, 4.00, 5.00 — and *not* linear: the gridline spacing measures about 34pt from 5 to 4, 44pt from 4 to 3 and 61pt from 3 to 2, so the axis is progressively stretched toward lower loss. Legend, nine dashed curves plus one additional dashed curve: "6e+18", "1e+19", "3e+19", "6e+19", "1e+20", "3e+20", "6e+20", "1e+21", "3e+21", and "Gopher" (all rendered as dashed lines in the same light-teal-to-black colormap as the left panel's vertical lines, with "Gopher" drawn as a thick black dashed line). Each series is a shallow parabola (dots = measured runs, dashed line = the parametric fit) with a visible minimum: the lightest/topmost series (6e+18) has its minimum around 200-300M parameters at loss ≈ 2.95-3.0; each successively darker series' minimum shifts right and down, with the near-black "3e+21" series bottoming out around 3-6B parameters at loss ≈ 2.15-2.2. The separate "Gopher" dashed curve sits below and to the right of all nine FLOP-budget series. Unlike them it shows no minimum within the plotted range — it is still descending at the right edge of the axis (40B parameters), so the chart does not display where it bottoms out. It is a projection at Gopher's own (larger) compute budget rather than one of the nine measured IsoFLOP slices.
...
- "Hoffmann et al." (green): dots span roughly -0.10 to **+0.125**, clearly clustered below zero but with two high outliers measured at +0.115 and +0.125 — these are the points the axis note above flags as sitting above 0.10, and an earlier version of this entry capped the range at +0.06, contradicting its own note; a short solid blue horizontal line marks the group's mean at about -0.047; the violin's widest point sits around -0.03 to -0.05.
- "Ours" (blue): dots cluster tightly between about -0.03 and +0.04, with one low outlier at -0.051, one borderline point at +0.064, and **two** high outliers measured at +0.154 and +0.169; the violin is narrow and centred almost exactly on 0.
...
- "Optimal policy (Hoffmann et al.)" (green): starts at about the same 20-25 tokens/param at low compute, but rises steadily and steeply, crossing the blue line at about $7$-$9\times10^{18}$ FLOPs (blue is still above green at $6.8\times10^{18}$, green clearly above by $9.1\times10^{18}$) and continuing up to roughly 200+ by $10^{27}$-$10^{28}$. Its confidence band (dashed green bounds) stays narrow throughout, hugging the solid green line.
- "Chinchilla model" (black dot): sits at approximately ($6.2\times10^{23}$, 20.15) — consistent with the 5.76e23 FLOP Gopher/Chinchilla budget this deck quotes on slide 47, right on the grey "D/N = 20" line.
